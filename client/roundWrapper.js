class RoundWrapper {
    constructor($rootscope, gameWrapper, globalSettings, $timeout) {
        this.remainingItems = [];
        this.currentHand = [];
        this.justDrawnCards = [];
    ...
    forceWaitingOnServer() { this.waitingOnServer = true; }
    isWaitingOnOthers() { return this.waitingOnServer; }
    ...
    getExtraStartingPoint(playerIndex) {
    ...
    drawHand(player, handSize = undefined) {
    ...
    clearExtraCards(item = undefined) {
    ...
    wasPreviousCardOfType(effect, spacesBack = 1) {
    ...
    showSelectionTurnOptions(playerData) {
    ...
    endSelectionTurn(didNotBust, ranOutOfTime = false) {
        if (this.waitingOnServer)
            return;
    ...
    getSpacePoints(index) {
        return Math.floor(index / 3.3) + 1;
    }
    endBuyTurn(ranOutOfTime = false) {
        if (this.waitingOnServer)
            return;
    ...
    scrollToIndex(index) {
        ...
        if (boardAreaElem) {
            const itemElem = document.getElementsByClassName("sw-boardSpace")[index]; // hopefully this is it...
    ...
    haveBoughtEnhancement(enhancement) {
        return this.buySelection.enhancements.findIndex(e => e == enhancement) >= 0;
    }
    refundEnhancement(enhancementDupeNdx, enhancement) {
    ...
    buyEnhancement(enhancement, cost, player) {
    ...
    canBuyEnhancement(enhancement, cost, player) {
        if (enhancement == purchaseEnhancement.CanBuyADuplicate) {
    ...
    getAdditionalEnhancementCost(enhancement) {
        // These don't cost extra to buy.
        if (enhancement >= purchaseEnhancement.ExtraMoney)
            return 0;
        return this.enhancementExtraCost;
    }
    getPercentageToBust(player) {
    ...
    getPercentageForOneCard(player) {
        if (this.remainingItems.length == 0)
            return 0;
        let cardsToDraw = this.handSize - this.currentHand.length;
        let percentage;
...
class TimerHelper {
    ...
    startPreRoundCountdown(callback, timeoutMax) {
    ...
    startRoundTimer(callback, timeoutMax) {
...
TimerHelper.tickAmount = 100;
//# sourceMappingURL=roundWrapper.js.map