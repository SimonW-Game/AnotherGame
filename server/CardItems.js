...
exports.getEnhancementCost = exports.getAllItemsOfEffect = exports.getItem = void 0;
const Game_1 = require("./Game");
function getItem(effect, points = 1) {
    points = Math.min(Math.max(1, points), 4);
    let item = { amount: points, effect: effect, points: points, cost: points };
    /*
     * Approximate cost:
     ...
     */
    if (effect == Game_1.itemEffect.JustMove) {
        item.cost = item.points * 4;
        if (item.points >= 3)
            item.cost++;
    }
    else if (effect == Game_1.itemEffect.SpecialNoEffect) {
        item.amount = 1;
        item.points = 1;
        item.cost = 5;
    }
    else if (effect == Game_1.itemEffect.AddToHand) {
        if (points == 1)
            item.cost = 6;
        else if (points == 2)
            item.cost = 11;
    ...
    else if (effect == Game_1.itemEffect.MoneyForSpecial) {
        item.amount = 1;
        item.points = 1;
        item.cost = 7;
    }
    else if (effect == Game_1.itemEffect.MovesForSpecial) {
        item.amount = 1;
        item.cost = ((2 + points) * 4); // 12,16,18,23
        if (item.cost == 4)
            item.cost++;
    }
    else if (effect == Game_1.itemEffect.BonusForKeys) {
        // You have to have keys in your deck which means you'd be spending turns buying cards that do nothing.
        item.amount = 1;
        item.points = 0;
        item.cost = 9;
    }
    else if (effect == Game_1.itemEffect.FarMoving) {
        item.amount = 1;
        item.points = 6;
        item.cost = 26; // vanilla $$ to movement (4*6) + $2 because holy jeeze.
    }
    else if (effect == Game_1.itemEffect.GemLandingExtra) {
    ...
    else if (effect == Game_1.itemEffect.ShuffleHand) {
    ...
    else if (effect == Game_1.itemEffect.DrawLowestNonBane) {
    ...
    else if (effect == Game_1.itemEffect.GrowingMover) {
    ...
    else if (effect == Game_1.itemEffect.GrowingPoints) {
    ...
    else if (effect == Game_1.itemEffect.CopyOfPreviousCardToHand) {
        // This card is overpowered.
    ...
    else if (effect == Game_1.itemEffect.CardsCostLess) {
    ...
    else if (effect == Game_1.itemEffect.LastCardGem) {
    ...
    else if (effect == Game_1.itemEffect.Virus) {
        item.amount = 1;
        item.points = 0;
        item.cost = 0;
    }
    else if (effect == Game_1.itemEffect.Poison) {
        item.amount = 1;
        item.points = 0;
        item.cost = 0;
    }
    else if (effect == Game_1.itemEffect.Tax) {
        item.amount = 1;
        item.points = 0;
        item.cost = 0;
    }
    else if (effect == Game_1.itemEffect.PoisonBrewer) {
        // Paying $5 for making people more prone to bust for the rest of the game.
        item.amount = 1;
        item.points = 1;
        item.cost = 9;
    }
    else if (effect == Game_1.itemEffect.TaxCollector) {
        // Paying $4 for making people have a smaller hand size or less money.
        item.amount = 1;
        item.points = 2;
        item.cost = 12;
    }
    else if (effect == Game_1.itemEffect.BaneGiver) {
        // Paying $12 for making people suffer, you dirty dog, you.
        item.amount = 1;
        item.points = 1;
        item.cost = 16;
    }
    else if (effect == Game_1.itemEffect.TrashItem) {
    ...
    else if (effect == Game_1.itemEffect.DiscardItem) {
    ...
    else if (effect == Game_1.itemEffect.Move5X) {
    ...
    else if (effect == Game_1.itemEffect.BaneCountRemoval) {
    ...
    else if (effect == Game_1.itemEffect.GainExtraMoney) {
        item.points = 1;
        item.amount = Math.min(4, points);
        item.cost = 3 + (item.amount * 3); // 6,9,12,15
    }
    else if (effect == Game_1.itemEffect.GainPoints5X) {
    ...
    else if (effect == Game_1.itemEffect.Bane1Moves2) {
    ...
    else if (effect == Game_1.itemEffect.MovesForGems) {
    ...
    else if (effect == Game_1.itemEffect.EmptyHandGems) {
        // 3 gems is worth around 4-5, so if you can hit this, you'll have 4+4.5=8.5 worth of a card.
        item.points = 1;
        item.amount = 1;
        item.cost = 6;
    }
    else if (effect == Game_1.itemEffect.EmptyHandMoves) {
        item.points = Math.min(2, points);
        item.amount = item.points;
        item.cost = 2 + (4 * item.points);
    }
    else if (effect == Game_1.itemEffect.IncreaseHandSize) {
    ...
    else if (effect == Game_1.itemEffect.GainPointPerBuy) {
    ...
    else if (effect == Game_1.itemEffect.MoveAndPoint) {
    ...
    else if (effect == Game_1.itemEffect.MoveNextGem) {
    ...
    else if (effect == Game_1.itemEffect.GemsForMoney) {
...
function getAllItemsOfEffect(effect) {
    let maxSize = 0;
    switch (effect) {
        case Game_1.itemEffect.Virus:
        case Game_1.itemEffect.Tax:
        case Game_1.itemEffect.Poison:
            maxSize = 0;
            break;
        case Game_1.itemEffect.SpecialNoEffect:
        case Game_1.itemEffect.MoneyForSpecial:
        case Game_1.itemEffect.FarMoving:
        case Game_1.itemEffect.ShuffleHand:
        case Game_1.itemEffect.GrowingMover:
        case Game_1.itemEffect.GrowingPoints:
        case Game_1.itemEffect.CopyMover:
        case Game_1.itemEffect.SpecialAdjacentMover:
        case Game_1.itemEffect.LastCardGem:
        case Game_1.itemEffect.PoisonBrewer:
        case Game_1.itemEffect.TaxCollector:
        case Game_1.itemEffect.BaneGiver:
        case Game_1.itemEffect.TrashItem:
        case Game_1.itemEffect.DiscardItem:
        case Game_1.itemEffect.PointInvestment:
        case Game_1.itemEffect.BonusForKeys:
        case Game_1.itemEffect.Bane1Moves2:
        case Game_1.itemEffect.EmptyHandGems:
        case Game_1.itemEffect.IncreaseHandSize:
        case Game_1.itemEffect.GainPointPerBuy:
        case Game_1.itemEffect.GainExtraGemFromHere:
        case Game_1.itemEffect.PlayedMostReward:
        case Game_1.itemEffect.DrawLowestNonBane:
        case Game_1.itemEffect.MoveNextGem:
        case Game_1.itemEffect.PointsForNoGems:
            maxSize = 1;
            break;
        case Game_1.itemEffect.GemLandingExtra:
        case Game_1.itemEffect.CopyOfPreviousCardToHand:
        case Game_1.itemEffect.CardsCostLess:
        case Game_1.itemEffect.GainExtraBuy:
        case Game_1.itemEffect.BaneCountRemoval:
        case Game_1.itemEffect.GainPoints5X:
        case Game_1.itemEffect.EmptyHandMoves:
        case Game_1.itemEffect.MoneyForPassingGems:
            maxSize = 2;
            break;
        case Game_1.itemEffect.MovesForGems:
        case Game_1.itemEffect.GemsForMoney:
            maxSize = 3;
            break;
        case Game_1.itemEffect.MovesForSpecial:
        case Game_1.itemEffect.AddToHand:
        case Game_1.itemEffect.RemovePreviousBane:
        case Game_1.itemEffect.Move5X:
        case Game_1.itemEffect.GainExtraMoney:
        case Game_1.itemEffect.MoveAndPoint:
        case Game_1.itemEffect.JustMove:
            maxSize = 4;
            break;
    }
    return new Array(maxSize).fill(undefined).map((_, ndx) => getItem(effect, ndx + 1));
}
exports.getAllItemsOfEffect = getAllItemsOfEffect;
function getEnhancementCost(enhancement, options) {
    if (enhancement == Game_1.purchaseEnhancement.IncreaseStartingSquare)
        return 8;
    else if (enhancement == Game_1.purchaseEnhancement.IncreaseHandSize)
        return 16;
    else if (enhancement == Game_1.purchaseEnhancement.CanBuyADuplicate)
        return 5;
    else if (enhancement == Game_1.purchaseEnhancement.GainPoint)
        return Math.max(1, Math.min(8, options.endGameGemsForPoints - 2));
    else if (enhancement == Game_1.purchaseEnhancement.GainTwoPoints)
        return Math.max(2, Math.min(16, (options.endGameGemsForPoints - 2) * 2));
    else if (enhancement == Game_1.purchaseEnhancement.GainThreePoints)
        return Math.max(3, Math.min(24, (options.endGameGemsForPoints - 2) * 3));
    else if (enhancement == Game_1.purchaseEnhancement.IncreaseBaneThreshold)
        return 15;
    else if (enhancement == Game_1.purchaseEnhancement.VirusSpreader)
        return 13;
    else if (enhancement == Game_1.purchaseEnhancement.ExtraMoney)
        return 12;
    else if (enhancement == Game_1.purchaseEnhancement.RefreshPerk)
        return 5;
    return 20; // In case I missed something I guess /shrug
}
exports.getEnhancementCost = getEnhancementCost;
//# sourceMappingURL=CardItems.js.map