...
const gameHelper = require("../server/Game");
function setupSocket(io, socket) {
    try {
            ...
            if (typeof game === "undefined") {
                ...
                if (game.hasGameNotStarted()) {
                    ...
                    if (player === undefined) {
                        ...
                        currentGameId = gameId;
                        socket.join(gameId);
                        const selection = game.round.playerSelections[player.playerData.index];
                        const buySelection = game.round.playerBuySelections[player.playerData.index];
        ...
        socket.on("startGame", function (gameId, options) {
            try {
                const game = gameHelper.getGame(gameId);
                if (typeof game !== "undefined") {
        ...
        socket.on("updateOptions", function (gameId, options) {
            try {
                const game = gameHelper.getGame(gameId);
                if (typeof game !== "undefined") {
                    game.options = options;
        ...
        socket.on("setOptionMode", function (gameId, modeIndex) {
            try {
                const game = gameHelper.getGame(gameId);
                if (typeof game !== "undefined") {
        ...
        socket.on("finishSelecting", function (gameId, selection) {
            const game = gameHelper.getGame(gameId);
            if (typeof game !== "undefined") {
        ...
        socket.on("finishBuying", function (gameId, buySelection) {
            const game = gameHelper.getGame(gameId);
            if (typeof game !== "undefined") {
                const player = game.getPlayer(socket);
